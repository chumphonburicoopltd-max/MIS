...
// --- ตั้งค่าระบบระบบสารสนเทศส่วนกลาง (ดึงข้อมูลจาก Script Properties เพื่อความปลอดภัยเมื่ออัปโหลดขึ้น GitHub) ---
...
const SITE_URL = PropertiesService.getScriptProperties().getProperty('SITE_URL') || "https://sites.google.com/view/management-information-system5"; 
...
function doGet(e) {
  ...
  if (page === 'admin') {
    ...
    // ปรับปรุง: เนื่องจากไม่มีไฟล์ Index จึงเปลี่ยนเส้นทาง (Redirect) ไปหน้าหลัก Google Sites โดยตรง
    const redirectHtml = `
      <!DOCTYPE html>
      <html>
        <head>
        ...
        <body>
          <div style="font-family: Arial, sans-serif; text-align: center; margin-top: 100px;">
            <h2>กำลังเชื่อมโยงไปยังระบบหลัก...</h2>
            <p>หากไม่มีการนำทางอัตโนมัติภายใน 3 วินาที <a href="${SITE_URL}" target="_top">คลิกที่นี่</a></p>
          </div>
        </body>
      </html>
    `;
...
function getMasterAdminKey() {
  ...
  if (!masterKey) {
    throw new Error("ระบบความปลอดภัยปฏิเสธ: ไม่พบรหัสผ่านสูงสุด (ADMIN_MASTER_KEY) ในระบบตั้งค่า Script Properties");
  }
  return masterKey;
}
